...
public enum class WasmMediaErrorCategory {
    SOURCE,
    NETWORK,
    FORMAT,
    DECODER,
...
public enum class WasmMediaErrorCode {
    INVALID_SOURCE,
    SOURCE_UNAVAILABLE,
    RANGE_UNSUPPORTED,
    UNSUPPORTED_FORMAT,
    ...
    OUTPUT_REQUIREMENT_UNAVAILABLE,
    PROTECTED_CONTENT_TRANSFORM_UNSUPPORTED,
    THUMBNAIL_FAILED,
    UNKNOWN,
}

public class WasmMediaError(
    public val category: WasmMediaErrorCategory,
    public val code: WasmMediaErrorCode,
    message: String,
    cause: Throwable? = null,
) : Exception(redactSensitiveText(message), cause) {
    override fun toString(): String = "WasmMediaError(category=$category, code=$code, message=${message.orEmpty()})"
}
