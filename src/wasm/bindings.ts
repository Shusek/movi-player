...
export class WasmBindings {
  ...
  getAttachments(maxBytes: number = 32 * 1024 * 1024): WasmAttachment[] {
    if (!this.contextPtr) return [];
    ...
    const count = getCount(this.contextPtr);
    const result: WasmAttachment[] = [];
    ...
    try {
      for (let index = 0; index < count; index++) {
        ...
        try {
          ...
          data.set(
            this.module.HEAPU8.subarray(dataPtr, dataPtr + copied),
          );
          result.push({
